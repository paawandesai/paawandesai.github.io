...
function AbacusComponent() {
  useEffect(() => {
    // Function to initialize and run the abacus
    function runAbacus() {
        ...
        this.init = function () {
          ...
          for (var i = 0; i < this.beadLines; i++) {
            for (var j = 0; j < this.beadPerLine; j++) {
              // Adjust bead positions to fit the new configuration
              var bead = new Bead();
              bead.position[0] = 580 - i * this.beadSpacing;
              bead.position[1] = 60 + this.beadPerLine * this.beadHeight - j * this.beadHeight;
              bead.value = 1;
              if (j > this.beadSep) {
                bead.position[1] = 60 + (this.beadPerLine - this.beadSep) * this.beadHeight - (j - this.beadSep) * this.beadHeight;
                bead.value = 5;
              }
              bead.uniqueID = id;
      ...
      // Instantiate and initialize the abacus
      const abacus = new Abacus("myAb", 0);
      abacus.init();
    }

    // Run the abacus when the component mounts
    runAbacus();

    // Clean up on component unmount
    return () => {
      // Perform any cleanup or teardown if necessary
    };
